...
class Message(models.Model):
    ...
    debts=models.ManyToManyField(get_model(conf.STUDENT_DEBT),related_name="messages",blank=True)
    created_date=models.DateTimeField(auto_now_add=True)
    sended_date=models.DateTimeField(auto_now=True)
    
    @property
    def content(self):
        from conf.utils.sms import get_kirish_content,get_chiqish_content,get_debts_content
